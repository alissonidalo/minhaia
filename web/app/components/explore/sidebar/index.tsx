...
const SideBar: FC<IExploreSideBarProps> = ({
  ...
  // Filtrando aplicativos com base no termo de busca
  const filteredApps = installedApps.filter(({ app: { name } }) =>
    ...
    <div className='w-fit sm:w-[216px] shrink-0 pt-6 px-4 border-gray-200 cursor-pointer'>
      <div>
        <Link
          href='/explore/apps'
          className={cn(
            isDiscoverySelected ? 'text-primary-600  bg-white font-semibold' : 'text-gray-700 font-medium hover:bg-gray-200',
      ...
      {installedApps.length > 0 && (
        <div className='mt-10'>
          ...
          >
            {filteredApps.map(({ id, is_pinned, uninstallable, app: { name, icon, icon_background } }) => {
              return (
                <Item
                  key={id}
                  isMobile={isMobile}
